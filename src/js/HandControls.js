...
export class HandControls extends THREE.EventDispatcher {
  ...
  update(landmarks) {
    if (landmarks.multiHandLandmarks.length === 1) {
      ...
      const handRight = new THREE.Vector3().crossVectors(handUp, handDirection).normalize();
      handUp.crossVectors(handDirection, handRight).normalize();
      const rotationMatrix = new THREE.Matrix4().makeBasis(handRight, handUp, handDirection.normalize());
